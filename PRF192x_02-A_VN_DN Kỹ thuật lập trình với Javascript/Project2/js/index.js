...
function insertInfo() {
	...
	if (
		...
		!between(testScore.math, 0, 10) ||
		!between(testScore.physical, 0, 10) ||
...
function calcAverage() {
	...
	for (var i = 1; i < numOfRow; i++) {
		...
		var avg =
			(parseFloat(cell_array[2].innerHTML) +
				parseFloat(cell_array[3].innerHTML) +
				parseFloat(cell_array[4].innerHTML)) /
			3;
	...
	if (parseInt(n) < table.rows.length && parseInt(n) != 0) {
		...
		for (var i = n; i < table.rows.length; i++) {
			table.rows[i].cells[0].innerHTML =
				parseInt(table.rows[i].cells[0].innerHTML) - 1;
		}
	}
}
